    ...
    async function (req, res) {
        if (!req.session.userId) {
            ...
            console.log(`Start: ${formatISO(weekStart)}`);
                ...
                try {
                    // Get the events
                    const mailevents = await graph.getMailEvents(
                    ...
                    params.mailevents = mailevents.value;
                } catch (err) {
                    res.send(JSON.stringify(err, Object.getOwnPropertyNames(err)));
                }
            }
            res.render('outlook', params);
        }
);

/* GET /outlook/newmessage */
...
router.post('/new', [
    ...
    body('ev-toRecipients').customSanitizer(value => {
        return value.split(';');
    ...
    if (!req.session.userId) {
        ...
        const formData = {
            ...
            toRecipients: req.body['ev-toRecipients'],
        ...
        // Create the event
        try {
            await graph.createEvent(msalClient, formData, user.timeZone);
        } catch (error) {
            req.flash('error_msg', {
        ...
        // Redirect back to the outlook view
        return res.redirect('/outlook');
    }
}
);
